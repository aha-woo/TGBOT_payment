...
async def invite_user_to_channel(app: Application, user_id: int, order_id: str) -> bool:
    ...
    try:
        ...
        # member_limit=1 确保链接只能使用一次，点击后立即失效
        # 不设置 expire_date，链接永久有效但只能用一次
        invite_link = await app.bot.create_chat_invite_link(
            chat_id=PRIVATE_CHANNEL_ID,
            member_limit=1  # 一次性链接，使用后立即失效
        )
...
1️⃣ 点击下方「🛒 打开闲鱼商品」按钮
   → 跳转到闲鱼商品页面
2️⃣ 在闲鱼购买商品
   商品名称：【🔥AI投资新视野｜专属课程+编程服务｜零基础直达高手】
   支付金额：¥{plan_info['price_cny']}
3️⃣ 购买成功后，复制闲鱼订单号
   示例：20231024123456789
4️⃣ 回到这个聊天窗口，直接发送订单号
   ⚠️ 就是在下方输入框输入订单号，然后发送给我
5️⃣ 等待管理员审核（人工审核，24小时内处理）
   审核通过后会发送邀请链接给您
...
1️⃣ 点击下方「🛒 前往闲鱼支付」按钮
   → 跳转到闲鱼商品页面
2️⃣ 在闲鱼购买商品、商品名称：【🔥AI投资新视野｜专属课程+编程服务｜零基础直达高手】
   支付金额：¥{plan_info['price_cny']}
3️⃣ 购买成功后，复制闲鱼订单号
   示例：20231024123456789
4️⃣ 回到这个聊天窗口，直接发送订单号
